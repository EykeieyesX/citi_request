...
$crequest_number = isset($_GET['crequest_number']) ? $_GET['crequest_number'] : '';

// Verify the request exists and belongs to the user
$username = $_SESSION['username'];
$sql = "SELECT * FROM barangay_clearance WHERE crequest_number = ? AND username = ? AND status = 'completed'";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ss", $crequest_number, $username);
$stmt->execute();
$result = $stmt->get_result();
$crequest = $result->fetch_assoc();
...
$feedback_check_sql = "SELECT * FROM feedback_clearance WHERE crequest_number = ? AND username = ?";
$feedback_check_stmt = $conn->prepare($feedback_check_sql);
$feedback_check_stmt->bind_param("ss", $crequest_number, $username);
...
if (!$feedback_exists && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insert_sql = "INSERT INTO feedback_clearance (crequest_number, username, accessibility, timeliness, user_experience, complaint, feedback_date) 
                   VALUES (?, ?, ?, ?, ?, ?, NOW())";
    $insert_stmt = $conn->prepare($insert_sql);
    $insert_stmt->bind_param("ssiiis", $crequest_number, $username, $accessibility, $timeliness, $user_experience, $complaint);
...
<div class="feedback-container">
    <div class="feedback-header">
        <h2>Feedback for Request #<?php echo htmlspecialchars($crequest_number); ?></h2>
    ...
    <div class="feedback-form" style="display: <?php echo $feedback_exists ? 'none' : 'block'; ?>">
        <form method="POST" action="feedback_clearance.php?crequest_number=<?php echo htmlspecialchars($crequest_number); ?>">
    ...
    <div class="feedback-view">
        ...
        <div class="rating-section">
            ...
            <div class="star-display">
                ...
                for ($i = 1; $i <= 5; $i++): ?>
                    <span class="star <?php echo ($i <= $accessibility) ? 'star-filled' : 'star-empty'; ?>">★</span>
                <?php endfor; ?>
            </div>
        ...
        <div class="rating-section">
            ...
            <div class="star-display">
                ...
                for ($i = 1; $i <= 5; $i++): ?>
                    <span class="star <?php echo ($i <= $timeliness) ? 'star-filled' : 'star-empty'; ?>">★</span>
                <?php endfor; ?>
            </div>
        ...
        <div class="rating-section">
            ...
            <div class="star-display">
                ...
                for ($i = 1; $i <= 5; $i++): ?>
                    <span class="star <?php echo ($i <= $user_experience) ? 'star-filled' : 'star-empty'; ?>">★</span>
                <?php endfor; ?>
            </div>
